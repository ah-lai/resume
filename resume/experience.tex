...
\begin{siderules}
  ...
  \begin{cventries}
    ...
    {
      \begin{cvitems}
        \item{Designed and developed an analytics library using \textbf{jQuery} and Segment API; analyzing user behavior for product improvements by capturing \textbf{20} unique events across \textbf{300+} daily users}
        ...
        \item {Developed a microservice using \textbf{React} and \textbf{AWS Lambda (NodeJS)} to upload and store study videos into \textbf{AWS S3}; increasing upload file size from 5GB to \textbf{5TB} and improving upload performance }
        \item {Built a support tool feature in an \textbf{ASP.NET} application to search and clone studies; decreasing support resolve times by \textbf{20\%} }
      \end{cvitems}
    }
    \cventry
    ...
    {
      \begin{cvitems}
        \item {Eliminated legacy application’s security vulnerabilities by leveraging \textbf{AWS IAM} roles and \textbf{AWS Parameter}; automating role $\>$ $\>$ $\>$ $\>$ assignments and EC2 instantiation using \textbf{Terraform}}
        \item {Developed an event-driven service to monitor user login activity using \textbf{AWS Lambda (NodeJS)}, \textbf{EventBridge}, and \textbf{Datadog}}
        ...
        \item {Created automated unit tests with NUnit framework, increasing test coverage by \textbf{25\%}; ensuring functionality is preserved}
      \end{cvitems}
    }
    \cventry
